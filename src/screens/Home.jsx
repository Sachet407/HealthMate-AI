...
const Home = () => {
  ...
  return (
 // Update background and text classes based on dark mode
<div className={`min-h-[calc(100vh-7rem)] w-full overflow-hidden `}>
  {/* Hero Section */}
  <motion.div
    className={`relative pt-16 pb-32 px-4 md:px-10 ${darkMode ? "text-gray-100" : ""}`}
    variants={heroVariants}
    initial="hidden"
    animate="visible"
  >
    {/* Background Shapes - Adjust opacity and colors for dark mode */}
    <motion.div
      className={`absolute top-0 right-0 -z-10 h-64 w-64 rounded-full ${darkMode ? "opacity-30 bg-blue-700" : "opacity-20 bg-blue-300"} blur-3xl`}
      animate={{
        x: [20, -20],
        y: [0, -30, 0],
      }}
    ...
    />
    <motion.div
      className={`absolute bottom-10 left-10 -z-10 h-96 w-96 rounded-full ${darkMode ? "opacity-30 bg-purple-700" : "opacity-20 bg-purple-300"} blur-3xl`}
      animate={{
        x: [-20, 20],
        y: [0, 30, 0],
      }}
    ...
    <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-12">
      ...
      >
        ...
        >
          <span className={`${darkMode ? "bg-blue-900 text-blue-200" : "bg-blue-100 text-blue-800"} text-xl font-medium px-4 py-1.5 rounded-full`}>
            Next Generation Healthcare
          </span>
          <motion.span
            className={`absolute inset-0 ${darkMode ? "bg-blue-900" : "bg-blue-100"} rounded-full -z-10`}
        ...
        <motion.p
          variants={itemVariant}
          className={`text-lg md:text-xl ${darkMode ? "text-gray-300" : "text-slate-600"} mb-10 max-w-2xl mx-auto lg:mx-0 leading-relaxed`}
        >
        ...
        >
          <Link to="/diagnosis">
            <motion.button
              ...
              className={`px-8 py-4 ${
                darkMode 
                  ? "bg-gradient-to-r from-blue-500 to-indigo-500 shadow-lg shadow-blue-900/30" 
                  : "bg-gradient-to-r from-blue-600 to-indigo-600 shadow-lg shadow-blue-200"
              } text-white rounded-xl text-lg font-medium flex items-center justify-center gap-2 group`}
            >
          ...
          <Link to="/about">
            <motion.button
              ...
              className={`px-8 py-4 ${
                darkMode 
                  ? "bg-gray-800 border-2 border-blue-800 text-blue-300" 
                  : "bg-white border-2 border-blue-200 text-blue-700"
              } rounded-xl text-lg font-medium shadow-md hover:shadow-lg transition-all duration-300`}
            >
              Learn More
            </motion.button>
          </Link>
      ...
      <motion.div className="flex-1 w-full" variants={itemVariant}>
        <div className="relative">
          <motion.img
            ...
            className={`w-full rounded-3xl ${darkMode ? "shadow-xl shadow-blue-900/20" : "shadow-2xl"} object-cover z-10 relative`}
    ...
    >
      <p className={`text-center text-sm font-medium ${darkMode ? "text-gray-400" : "text-slate-500"} mb-6`}>
        ...
        {[
          ...
          >
            <motion.img
              ...
              className={`h-10 object-contain mb-2 grayscale hover:grayscale-0 transition-all duration-300 ${darkMode ? "brightness-125" : ""}`}
            />
            <span className={`${darkMode ? "text-gray-400" : "text-slate-500"} text-sm font-medium`}>
              {partner.name}
            </span>
          </motion.div>
        ))}
  ...
  >
    <motion.div
      ...
      viewport={{ once: true }}
    >
      <h2 className={`text-3xl md:text-4xl font-bold ${darkMode ? "text-gray-100" : "text-slate-800"} mb-4`}>
        All-in-One Healthcare Platform
      </h2>
      <p className={`${darkMode ? "text-gray-300" : "text-slate-600"} text-lg max-w-2xl mx-auto`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {features.map((feature, i) => (
        <motion.div
          ...
          className={`${darkMode ? "bg-gray-800 border-gray-700" : `${feature.color} border ${feature.borderColor}`} p-8 rounded-2xl shadow-lg hover:shadow-xl transform transition-all duration-300`}
          whileHover={{
            y: -8,
            boxShadow:
              darkMode 
        ...
        >
          <div className="flex items-start gap-4">
            <div className={`mt-1 ${darkMode ? "text-blue-400" : ""}`}>{feature.icon}</div>
            <div>
              <h3 className={`text-xl font-semibold ${darkMode ? "text-white" : "text-slate-800"} mb-3`}>
                {feature.title}
              </h3>
              <p className={`${darkMode ? "text-gray-300" : "text-slate-600"} text-base leading-relaxed`}>
                {feature.text}
              </p>
            </div>
          </div>
    ...
    <div className={`max-w-5xl mx-auto ${
      ...
      <div className="relative p-12 md:p-16">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <motion.div
            className={`absolute -left-20 -bottom-20 w-64 h-64 rounded-full ${
              darkMode 
                ? "bg-gradient-to-tr from-purple-700 to-pink-600 opacity-15" 
                : "bg-gradient-to-tr from-purple-500 to-pink-400 opacity-25"
            } blur-md`}
        ...
        <div className="relative z-10 text-center">
          ...
          <motion.p
            className={`${darkMode ? "text-blue-200" : "text-blue-100"} text-lg mb-10 max-w-2xl mx-auto`}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <Link to="/signup">
              <motion.button
                ...
                className={`px-8 py-4 ${
                  darkMode 
                    ? "bg-gray-800 text-blue-300 shadow-xl hover:shadow-2xl shadow-black/30" 
      ...
      {[
        ...
        >
          <div className="flex flex-col h-full">
            <div className={`text-5xl ${darkMode ? "text-blue-400" : "text-blue-400"} mb-4`}>"</div>
            <p className={`${darkMode ? "text-gray-300" : "text-slate-700"} text-lg italic flex-grow mb-6`}>
              {testimonial.quote}
            </p>
            <div className="flex items-center mt-4">
              <motion.img
                ...
                className={`w-12 h-12 rounded-full object-cover border-2 ${darkMode ? "border-gray-600" : "border-blue-100"}`}
                whileHover={{ scale: 1.1, borderColor: darkMode ? "#60a5fa" : "#3b82f6" }}
                transition={{ duration: 0.3 }}
              />
              <div className="ml-4">
